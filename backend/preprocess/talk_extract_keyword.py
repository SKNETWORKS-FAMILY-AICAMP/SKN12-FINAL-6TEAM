...
# 처리할 파일 경로
INPUT_JSON_FILE = "./data/감성대화말뭉치(최종데이터)_Training.json"
OUTPUT_JSON_FILE = "result/ai_data.json"
...
async def classify_sentence_with_retry(client, semaphore, sentence):
    """
...
async def main():
    ...
    tasks = [classify_sentence_with_retry(client, semaphore, s) for s in sentences]
    
    results_with_num = []
    for future in tqdm_asyncio.as_completed(tasks, total=len(tasks), desc="감정 분류 진행"):
        result = await future
