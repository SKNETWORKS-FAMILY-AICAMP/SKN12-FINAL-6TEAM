      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-8">
        ...
        <div className="w-full max-w-md mx-auto mb-2 flex gap-3 items-center">
          <Input
            ...
            className="flex-1 h-[56px] px-6 rounded-full bg-white/90 backdrop-blur-sm border-0 text-gray-800 placeholder-gray-500 text-center text-lg focus:ring-2 focus:ring-purple-400"
          />
          <button
            onClick={handleNicknameCheck}
            disabled={isCheckingNickname || !nickname.trim()}
            className="bg-gradient-to-r from-pink-500 to-blue-600 hover:from-pink-600 hover:to-blue-700 disabled:from-gray-400 disabled:to-gray-500 text-white px-6 h-[56px] rounded-full text-base font-medium shadow-lg hover:shadow-xl transition-all duration-300 whitespace-nowrap"
          >
            {isCheckingNickname ? (
        ...
        <div className="mt-8 w-full max-w-md mx-auto">
          <button
            onClick={handleNext}
            disabled={!canProceed}
            className={`w-full h-[56px] text-lg font-medium rounded-full transition-all ${
              canProceed
        ...
        {/* Footer Text */}
        <p className="text-white/70 text-center text-sm mt-6 mb-8">
          마이페이지에서 언제든 다시 바꿀 수 있어요
        </p>
      </div>
    </div>
  )
